...
def perfect_square(score):
    if isqrt(score) ** 2 == score:
        return True
    return False


def next_perfect_square(score):
    return (isqrt(score) + 1) ** 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    fix_score = strategy(0, 0)
    for score in range(goal):
        for opponent_score in range(goal):
            if strategy(score, opponent_score) != fix_score:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num_roll in range(1, 11):
        ...
        point = average_dice(num_roll, dice)
        if point > max_point:
            max_point = point
            max_num_rolls = num_roll
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if perfect_square(score + tail_point):
        return 0
    result = tail_strategy(score, opponent_score, threshold, num_rolls)
    return result
    # END PROBLEM 11


average_dice = make_averaged(roll_dice)
aver_always_roll_score = average_dice(5)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    num_roll = square_strategy(score, opponent_score, aver_always_roll_score)
    if num_roll > 3 and score > opponent_score:
        return num_roll - 2
    return num_roll
    # END PROBLEM 12
