...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    select_para = [p for p in paragraphs if select(p) is True]
...
def about(subject):
    ...
    def check_in(para):
        for s in subject:
            if s in split(lower(remove_punctuation(para))):
                return True
        return False
...
def accuracy(typed, source):
    ...
    check_len = min(len(typed_words), len(source_words))

    count = 0
    for t, s in zip(typed_words[:check_len], source_words[:check_len]):
        if t == s:
            count += 1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    score = [diff_function(typed_word, word, limit) for word in word_list]
    min_score = min(score)
    if min_score > limit:
        return typed_word
...
def minimum_mewtations(typed, source, limit):
    ...
    if (
        typed in source or source in typed
...
def final_diff(typed: str, source: str, limit):
    ...
    if (
        ...
        # END
    if len(typed) >= 2:
        if typed[1] + typed[0] == source[0:1]:
            return 1 + final_diff(typed[1] + typed[0] + typed[2:], source, limit - 1)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    d = dict()
    d["id"] = user_id
    d["progress"] = len(typed) / len(prompt)

    for i in range(len(typed)):
        if typed[i] != prompt[i]:
            d["progress"] = i / len(prompt)
            break
    upload(d)
    return d["progress"]
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[b - a for a, b in zip(l[:-1], l[1:])] for l in times_per_player]
...
def fastest_words(match):
    ...
    for word in word_indices:
        ...
        out[min_player_index].append(words[word])
    return out
    # END PROBLEM 10
