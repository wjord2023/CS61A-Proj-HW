...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        def helper(frame, symbol):
            value = frame.bindings.get(symbol, None)
            if value is not None:
                return value
            if frame.parent is not None:
                return helper(frame.parent, symbol)

        value = helper(self, symbol)
        if value is not None:
            return value
        # END PROBLEM 1
    ...
    def make_child_frame(self, formals, vals):
        ...
        while copy_formals is not nil:
            new_env.bindings[copy_formals.first] = copy_vals.first
