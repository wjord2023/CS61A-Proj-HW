...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        op = LambdaProcedure(formals, body, env)
        env.bindings[symbol] = op
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        out = scheme_eval(expressions.first, env)
        if is_scheme_true(out):
...
def make_let_frame(bindings, env):
    ...
    while copy_bindings is not nil:
        validate_form(copy_bindings.first, 2, 2)
        names = Pair(copy_bindings.first.first, names)
        vals = Pair(scheme_eval(copy_bindings.first.rest.first, env), vals)
        copy_bindings = copy_bindings.rest
