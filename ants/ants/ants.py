...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.is_container is True and self.ant_contained:
            if self.buffered is False:
                self.damage *= 2
                self.buffered = True
            if self.ant_contained.buffered is False:
                self.ant_contained.damage *= 2
                self.ant_contained.buffered = True
        else:
            if self.buffered is False:
                self.damage *= 2
                self.buffered = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        return random_bee(helper(self.place, 0))  # REPLACE THIS LINE
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            bee_toeat = random_bee(self.place.bees)
            if bee_toeat:
                bee_toeat.reduce_health(bee_toeat.health)
